...
DiscriminatorMVA::DiscriminatorMVA(std::string weights)
{
  ...
  reader_->AddVariable("abs(cosTheta)",&var_[11]);
  reader_->AddVariable("log(x1/ht)"   ,&var_[12]);
  reader_->AddVariable("log(x2/ht)"   ,&var_[13]);
...
//-------------------------------------------------------------
float DiscriminatorMVA::eval(float mqq,float dEtaqq,float dPhiqq,float btag0,float btag1,float qgl0,float qgl1,float qgl2,float qgl3,float softHt,int nSoftJets,float cosTheta,float x1,float x2,float ht)
{
  var_[0]  = mqq;
  var_[1]  = dEtaqq;
  var_[2]  = dPhiqq;
