...
struct PLOTTER {
    ...
    void Init(TString inFile, TString outFile) {
        ...
        for(int i = 0; i < nPer; ++i) {
            ...
            hBalEtaPtAllUp[i] = dynamic_cast<TH3D*>(fIn->Get(SF("hBalEtaPtUp_%c",per)));
            assert(hBalEtaPtAllUp[i]);
            hBalEtaPtAllDn[i] = dynamic_cast<TH3D*>(fIn->Get(SF("hBalEtaPtDn_%c",per)));
...
void PLOTTER::MeanAsym(int shift)
{
...
{
    PLOTTER plot;
    TString myOut = "plots/augAltM.pdf";
    plot.Init("histoDir/AugErrAltM/jetsB.root", myOut+"(");
