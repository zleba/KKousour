void DrawVariable(TString DIR,TString VAR,bool LOG,int REBIN,float XMIN,float XMAX,TString XTITLE,bool PRINT)
{
  gROOT->ForceStyle();
  const int N = 10;
  TString SAMPLE[N] = {"JetHT","TT","WJetsToQQ_HT-600ToInf","QCD_HT200to300","QCD_HT300to500",
                       "QCD_HT500to700","QCD_HT700to1000","QCD_HT1000to1500","QCD_HT1500to2000","QCD_HT2000toInf"};
  float XSEC[N]     = {1.0,832,95.14,1.74e+6,3.67e+5,2.94e+4,6.524e+03,1.064e+03,121.5,2.542e+01};
  float LUMI(1280.23);
  TFile *inf[N];
  TH1F  *h[N];
  int COLOR[N] = {kBlack,kYellow-10,kOrange,kBlue-10,kBlue-9,kBlue-8,kBlue-7,kBlue-6,kBlue-5,kBlue-4};
  ...
  for(int i=0;i<N;i++) {
    ...
    TH1F *hTriggerPass = (TH1F*)inf[i]->Get(DIR+"/TriggerPass");
    h[i]   = (TH1F*)inf[i]->Get(DIR+"/h_"+VAR);
    if (!h[i]) {
      cout<<"Histogram "<<"h_"+VAR<<" does not exist !!!"<<endl;
      break;
    } 
    hTriggerPass->SetDirectory(0);
    h[i]->SetDirectory(0);
    inf[i]->Close();
    h[i]->Rebin(REBIN);
    h[i]->Sumw2();
  ...
  float kfactor = 1.0;
  if (hQCD->Integral() > 0) { 
    kfactor = (h[0]->Integral()-h[1]->Integral()-h[2]->Integral())/hQCD->Integral();
  }  
  hQCD->Scale(kfactor);
  ...
  TLegend *leg = new TLegend(0.86,0.75,0.99,0.9);
  leg->SetFillColor(0);
  leg->SetTextFont(42);
